...
```
# Mario Raceway has the fastest average time
# Question 5
...
avg %>%
  ...
  labs(x = "Year", y = "Mean Record Time", title = "Trend in Record Times by Year")
```
# There does not seem to be a clear line of progression or regression in average record time over these years
# Question 8
...
records %>%
  ...
  labs(x = "Record Time", y = "Frequency", title = "Comparison of Times for Single and Three Lap Tracks") 
...
records %>%
  ...
  geom_histogram(alpha = .5, position = "identity" , bins = 30) +
  labs(x = "Time per Round", y = "Frequency", title = "Comparison of Time per Round for Single Lap and Three Lap Tracks" ) 
```
# Question 10
...
```{r}
meanTimeSystem = records %>%
  group_by(system_played) %>%
  summarize(mean_time = mean(time))
print(meanTimeSystem)

# Do you observe a difference across the two game systems?(-1)
