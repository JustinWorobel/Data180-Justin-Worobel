...
obj1 = X[1, ]
obj2 = X[2, ]
euc_distance = sqrt(sum((obj1 - obj2)^2))
rounded_euc = round(euc_distance, 2)
print(rounded_euc)
```
...
manhatDist = sum(abs(obj1 - obj3))
rounded_man = round(manhatDist, 2)
rounded_man
```
...
minkDist = sum(abs(obj2 - obj3)^3)^(1/3)
roundedMink = round(minkDist, 2)
roundedMink
```

# You should compute by hand (-1)
...
```{r}
VansMat = as.matrix(Vans93[,-1])
Vans_Euclidean = dist(VansMat, method = "euclidean")
...
# Which two Vans (give the models) have the shortest distance between them? Which two have the largest distance between them?

Vans_Euclidean_Rounded = round(Vans_Euclidean, 2)
minDist = min(Vans_Euclidean_Rounded[Vans_Euclidean_Rounded > 0])
maxDist = max(Vans_Euclidean_Rounded)
min = which(Vans_Euclidean_Rounded == minDist)
max = which(Vans_Euclidean_Rounded == maxDist)
print(min)
print(max)
```
...
```{r}
Vansmat = as.matrix(Vans93[, -1])
VansMan = dist(Vansmat, method = "manhattan")
...
min[D(O1, O2), D(O1,O3), D(O1, O5), D(O4, O2), D(O4, O3), D(O4, O5)]
min[1,2,8,1.6,0,3] = 0
...
b) Farthest-neighbor
max[D(O1, O2), D(O1, O3), D(O1, O5), D(O4, O2), D(O4, O3), D(O4, O5)]
max[1,9,8,1.6,0,9] = 9

c) Average-neighbor
(D(O1, O2) + D(O1, O3) + D(O1, O5) + D(O4, O2) + D(O4, O3) + D(O4, O5)) / 6
((1 + 9 + 8 + 1.6 + 0 + 3) / 6 = 22.6 / 6) = 3.77
...
numRow = nrow(vertebrate)
cat("vertebrate has ", numRow, "Rows")
print(vertebrate)
...
rownames(Ver_Euclidean) = colnames(Ver_Euclidean)
hclust_euclidean = hclust(as.dist(Ver_Euclidean))
plot(hclust_euclidean, main = "Clustered vertebrates, Euclidean,\n Linkage type", sub = "", xlab="Vertebrates", hang=-1)
```

(ii)using the Manhattan metric with single linkage and
```{r}
vertebrate_manhattan = as.matrix(dist(vertebrate[, -1]))
rownames(vertebrate_manhattan) = colnames(vertebrate_manhattan)
hclust_manhattan = hclust(as.dist(vertebrate_manhattan), method = "single")
plot(hclust_manhattan, main = "Clustered vertebrates, Manhattan,\n Linkage type", sub ="", xlab="Vertebrates", hang=-1)
```
...
```{r}
library(ggdendro)
dendEuc = as.dendrogram(hclust_euclidean)

ggdendro::ggdendrogram(dendEuc, rotate = TRUE, labels_track_height =.1)
```
